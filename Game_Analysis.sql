...
-- Player Details Table
INSERT INTO pd(P_ID, PName, L1_Status, L2_Status, L1_Code, L2_Code) 
VALUES
...
SELECT pd.P_ID, Dev_ID, PName, difficulty AS Difficulty_level
FROM pd
...
FROM TopScores
WHERE rank <= 5 
GROUP BY Dev_ID,difficulty,score;
...
FROM FirstLogin
WHERE login_rank = 1
GROUP BY P_ID,Dev_ID,start_time;
...
SELECT P_ID, start_time, stages_crossed,
       SUM(stages_crossed) OVER (PARTITION BY P_ID ORDER BY start_time) AS Cumulative_Stages_Crossed
FROM CumulativeSum
WHERE row_num > 1
GROUP BY P_ID,stages_crossed,start_time
ORDER BY P_ID,start_time;
...
FROM TopScores
WHERE rank <= 3
GROUP BY Dev_ID,P_ID,Total_Score;
...
BEGIN
    ...
    FROM TopHeadshots
    WHERE rank <= @n
	GROUP BY Dev_ID,difficulty,headshots_count,rank order by rank;
END;
